...
#define VAR_INDEX_CONTAINER_O2O_INITIALIZER { HASHTABLE_INITIALIZER }
...
struct var_index_container_o2o {
    struct hashtable ht_keys;
};
...
static inline int var_index_container_o2o_init(struct var_index_container_o2o *ic, const char *filename) {
    char idx_filename[PATH_MAX];
    if (PATH_MAX <= snprintf(idx_filename, PATH_MAX, "%s.idx", filename) ||
        0 > hashtable_open(&ic->ht_keys, 0, idx_filename, O_RDONLY))
        return -1;
...
static inline int var_index_container_o2o_lookup(struct var_index_container_o2o *ic, const char *key, size_t key_len, uint32_t *loc) {
    uint32_t rec_ofs = hashtable_lookup(&ic->ht_keys, key, key_len);
    if (0 == rec_ofs)
        return -1;
    *loc = *(uint32_t *)hashtable_get_val(&ic->ht_keys, rec_ofs);
    return 0;
}

static inline int var_index_container_o2m_close(struct var_index_container_o2m *ic) {
    return hashtable_close(&ic->ht_keys) | file_mapper_free(&ic->fm);
}

static inline int var_index_container_o2o_close(struct var_index_container_o2o *ic) {
    return hashtable_close(&ic->ht_keys);
}
